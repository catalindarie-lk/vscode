...
uint64_t metadata_seq_num[MAX_CLIENT_FILE_STREAMS];
...
void start_threads(){
    ...
    for(int index = 0; index < MAX_CLIENT_FILE_STREAMS; index++){
        ...
        if (file_transfer_thread[index] == NULL || file_transfer_event[index] == NULL){
...
int main() {
    ...
    start_threads();
        ...
        fprintf(stdout, "\rProgress File: %.2f, Progress Text: %.2f, Hash queue frames: %d", (float)(client.file_size - client.file_bytes_to_send) / (float)client.file_size * 100.0, 
        ...
        switch (received_frame_type) {
            ...
            case FRAME_TYPE_ACK:
                ...
                for(int i = 0; i < MAX_CLIENT_FILE_STREAMS; i++){
                    if(received_seq_num == metadata_seq_num[i] && op_code == STS_ACK){
...
unsigned int WINAPI file_transfer_thread_func(LPVOID lpParam){
    ...
    while(client.client_status == CLIENT_READY){
        ...
        metadata_seq_num[index] = get_new_seq_num();
        send_file_metadata(metadata_seq_num[index], client.session_id, file_id, client.file_size, FILE_FRAGMENT_SIZE, client.socket, &client.server_addr);
        ...
        while(remaining_bytes_to_send > 0){
            ...
            while (chunk_bytes_to_send > 0){
                ...
                if(frame_fragment_size < TEXT_FRAGMENT_SIZE){
                    memset(buffer + frame_fragment_size, 0, (uint32_t)TEXT_FRAGMENT_SIZE - frame_fragment_size);
                }

                send_file_fragment(get_new_seq_num(), 
                                    client.session_id, 
                                    file_id, 
...
unsigned int WINAPI message_send_thread_func(void *ptr){
    ...
    while(client.client_status == CLIENT_READY){
        ...
        while(remaining_bytes_to_send > 0){ 
            ...
            send_long_text_fragment(get_new_seq_num(), 
                                        client.session_id, 
                                        message_id, 
                                        client.message_len, 
                                        frame_fragment_offset, 
