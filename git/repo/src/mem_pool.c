...
void pool_init(MemPool* pool) {
    ...
    pool->next = (uint64_t*)malloc(pool->block_count * sizeof(uint64_t));
    if (pool->next == NULL) {
...
void pool_free(MemPool* pool, void* ptr) {
    ...
    if (index >= pool->block_count || !pool->used[index]) { // Corrected bounds check
        ...
        fprintf(stderr, "   Is within bounds (0 to %llu)? %s\n", pool->block_count - 1,
                        (index < pool->block_count) ? "Yes" : "No"); // Corrected condition
    ...
    // Optionally, clear the memory for safety (helps in debugging by making old data visible)
    // Note: You need to include <string.h> for memset.
    // #include <string.h>
...
void pool_destroy(MemPool* pool) {
    ...
    if (pool->memory != NULL) { // Added null check for safety
        free(pool->memory);
        pool->memory = NULL;
    }
